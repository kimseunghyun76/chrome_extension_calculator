class Calculator {
    constructor(previousOperandTextElement, currentOperandTextElement) {
        ...
        this.currentOperandTextElement.addEventListener('input', (e) => {
            // 붙여넣기나 자동완성이 아닌 직접 입력의 경우
            if (e.inputType !== 'insertFromPaste') {
                const text = this.currentOperandTextElement.innerText
                // console.log('직접 입력:', text)
                
                // 숫자와 소수점만 추출
                const cleanedText = text.replace(/[^0-9.]/g, '')
                // 소수점이 여러 개인 경우 첫 번째 소수점만 유지
                const validNumber = cleanedText.replace(/(\..*)\./g, '$1')
                
                if (validNumber) {
                    this.currentOperand = validNumber
                    this.updateDisplay()
                } else {
                ...
                range.selectNodeContents(this.currentOperandTextElement)
        ...
        // 키보드 이벤트 리스너 추가
        this.currentOperandTextElement.addEventListener('keydown', (e) => {
            // Enter 키 처리
            if (e.key === 'Enter') {
                e.preventDefault()
                this.currentOperandTextElement.blur()
            }
            // Backspace 키 처리
